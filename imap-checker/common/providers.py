#!/usr/bin/env python3
"""Suy ra IMAP/SMTP host theo domain email. Chỉ hỗ trợ Gmail và Yahoo (kể cả Yahoo Japan)."""
...
def resolve_smtp_host(email_addr):
    """Trả về (host, port) SMTP theo domain email — dùng để gửi mail (không phải check mail).

    Raise ValueError nếu domain không thuộc Gmail/Yahoo.
    """
    domain = _domain(email_addr)
    if domain in GMAIL_DOMAINS:
        return "smtp.gmail.com", SMTP_PORT
    if domain in YAHOO_SMTP_HOSTS:
        return YAHOO_SMTP_HOSTS[domain], SMTP_PORT
    raise ValueError(
        f"Domain '{domain}' chưa được hỗ trợ (chỉ Gmail và Yahoo) — email: {email_addr}"
    )
